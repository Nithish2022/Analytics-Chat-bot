...
if 'show_sample_prompt' not in st.session_state:
    st.session_state['show_sample_prompt'] = False
...
def initialize_agent(dataframes):
    ...
    field_descriptions = {
        ...
        'alarm_duration': 'Duration of the alarm(seconds)',
        ...
        'cycle_time': 'Time taken to complete one cycle of production(seconds)',
        'handling_time': 'Time taken to handle the part during production(seconds)',
        'run_time': 'Total run time of the machine(seconds)',
        'idle_time': 'Total idle time of the machine(seconds)',
        'stop_time': 'Total stop time of the machine(seconds)',
        ...
        'targetruntime': 'Target runtime for the production process(seconds)',
        'targetidletime': 'Target idle time for the production process(seconds)',
        'settingtime': 'Time taken to set up the machine for production(seconds)',
        ...
        'idle_start_time': 'Timestamp indicating when the machine became idle(seconds)',
        'idle_end_time': 'Timestamp indicating when the machine resumed operation(seconds)',
        'idle_duration': 'Duration for which the machine was idle(seconds)',
        ...
        'part_start_time': 'Timestamp indicating when production of a part started(seconds)',
        'part_end_time': 'Timestamp indicating when production of a part ended(seconds)',
        'part_cycle_time': 'Cycle time for producing a part(seconds)',
        'part_handling_time': 'Handling time for a part(seconds)',
        'part_run_time': 'Run time for producing a part(seconds)',
        'part_idle_time': 'Idle time during the production of a part(seconds)',
...
def show_sample_prompts():
    ...
    sample_prompts = ["Who is the operator associated with the highest frequency of occurrences?", 
                      "show me the pie chart for the top 10 machine.", 
                      "which machine has significant operational impact on 06/06/2024."]
    
    st.write("")
    
    # Display sample prompts horizontally
    col1, col2, col3 = st.columns(3)
    
    if col1.button(sample_prompts[0]):
            send_prompt(sample_prompts[0])

    if col2.button(sample_prompts[1]):
            send_prompt(sample_prompts[1])

    if col3.button(sample_prompts[2]):
            send_prompt(sample_prompts[2])
...
def main_page():
    ...
    if st.session_state['selected_csv'] != selected_csv:
        ...
        try:
            ...
            st.session_state['show_sample_prompt'] = True  # Show sample prompt button when a new CSV is loaded
        except Exception as e:
    ...
    if st.session_state['dataframes'] is not None:
        if st.session_state['is_first_loading']:
            if not st.session_state['initial_messages_added']:
                append_messages(role="user", content="Hey! 👋 ", type="string")
                append_messages(role="assistant", content="Hello! I'm your virtual assistant. I'm here to provide information about your shop floor! 🤗", type="string")
    ...
    if st.session_state['show_sample_prompt']:
        show_sample_prompts()
